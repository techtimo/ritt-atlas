...
# 4. status + results_pdf changed → "2 Änderungen: Status, Dokumente"
def test_status_and_documents_changed():
    old = base_event()
    new = base_event(status="abgesagt", results_pdf="ergebnis.pdf", wiki_touched=TOUCHED_NEW)
    result = compute_notifications([old], [new])
    assert len(result) == 1
    assert result[0]["body"] == "2 Änderungen: Status, Dokumente"
...
# 6b. PDF different filename → documents notification
def test_pdf_different_name_notification():
    old = base_event(announcement_pdf="alt.pdf")
    new = base_event(announcement_pdf="neu.pdf", wiki_touched=TOUCHED_NEW)
    result = compute_notifications([old], [new])
    assert len(result) == 1
    assert result[0]["body"] == "Neue Dokumente verfügbar"
